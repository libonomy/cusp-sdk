...
// Find the contributing lby at that height and burn the specified slashFactor
...
func (k Keeper) Slash(ctx sdk.Context, consAddr sdk.ConsAddress, infractionHeight int64, power int64, slashFactor sdk.Dec) {
	...
	// redelegations, as that lby has since unbonded
...
// the unbonding delegation had enough lby to slash
// (the amount actually slashed may be less if there's
// insufficient lby remaining)
func (k Keeper) slashUnbondingDelegation(ctx sdk.Context, unbondingDelegation types.UnbondingDelegation,
	...
	for i, entry := range unbondingDelegation.Entries {
		...
		// If unbonding started before this height, lby didn't contribute to infraction
		...
		// Calculate slash amount proportional to lby contributing to infraction
		...
		// according to lby held at time of infraction
...
func (k Keeper) slashRedelegation(ctx sdk.Context, srcValidator types.Validator, redelegation types.Redelegation,
	...
	for _, entry := range redelegation.Entries {
		...
		// If redelegation started before this height, lby didn't contribute to infraction
		if entry.CreationHeight < infractionHeight {
			continue
		}
