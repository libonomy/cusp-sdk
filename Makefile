...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/evdatsion/cusp-sdk/types"
	godoc -http=:6060
...
test_ledger_mock:
	@go test -mod=readonly `go list github.com/evdatsion/cusp-sdk/crypto` -tags='cgo ledger test_ledger_mock'

test_ledger: test_ledger_mock
	@go test -mod=readonly -v `go list github.com/evdatsion/cusp-sdk/crypto` -tags='cgo ledger'
...
devdoc:
	docker run -it -v "$(CURDIR):/go/src/github.com/evdatsion/cusp-sdk" -w "/go/src/github.com/evdatsion/cusp-sdk" devdoc:local bash
