...
import (
	...
	tmcli "github.com/libonomy/aphelion-staking/libs/cli"
)
...
func GetCommands(cmds ...*cobra.Command) []*cobra.Command {
	for _, c := range cmds {
		...
		c.Flags().String(FlagNode, "tcp://localhost:26657", "<host>:<port> to Aphelion RPC interface for this chain")
...
func PostCommands(cmds ...*cobra.Command) []*cobra.Command {
	for _, c := range cmds {
		...
		c.Flags().String(FlagFees, "", "Fees to pay along with transaction; eg: 10ulby")
		c.Flags().String(FlagGasPrices, "", "Gas prices to determine the transaction fee (e.g. 10ulby)")
		c.Flags().String(FlagNode, "tcp://localhost:26657", "<host>:<port> to aphelion rpc interface for this chain")
		c.Flags().Bool(FlagUseLedger, false, "Use a connected Ledger device")
